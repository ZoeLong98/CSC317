...
import { pool } from "./server/config/database.js"; // Assuming you have a database.js file for your database connection
import {
  getAllEvents,
  getEventbyId,
  myUpcomingEvent,
...
app.use(
  session({
    secret: "a-secret-key", // Replace with your own secret key
    resave: false,
    saveUninitialized: true,
    cookie: { secure: false, maxAge: 3600000 }, // Set to true if using HTTPS, maxAge is set to 1 hour
  })
);
...
app.get("/login", (req, res) => {
  if (req.session.userId) {
    res.sendFile(path.join(__dirname, "server/public", "logout.html"));
  } else {
...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length > 0) {
      ...
      res.redirect("/login"); // Redirect to events page
    } else {
      res.render("login", {
